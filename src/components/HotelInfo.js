...
const HotelInfo = () => {
  ...
  const loadAccessibilityData = async () => {
    ...
    const resp = await fetch(
      'https://uzvuoh1kik.execute-api.ap-southeast-1.amazonaws.com/Production/accessibilities'
    );
  ...
  const loadServicesData = async () => {
    // Query the API Gateway
    const resp = await fetch(
      'https://uzvuoh1kik.execute-api.ap-southeast-1.amazonaws.com/Production/services'
    );
    let jsonData = await resp.json();
